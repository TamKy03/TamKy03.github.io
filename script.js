(() => {
  ...
  const toggleTheme = () => {
    ...
    try {
      localStorage.setItem("v3-theme", root.dataset.theme);
    } catch (e) {}
  };
  ...
  const commands = [
    ...
    { label: "Copy email address", hint: "action", run: copyEmail },
    {
      ...
      run: () =>
        window.open(
          "https://www.linkedin.com/in/kytam0330/",
          "_blank",
          "noopener",
        ),
    },
    ...
    {
      label: "View other site versions",
      hint: "link",
      run: () => {
  ...
  const render = () => {
    ...
    list.innerHTML = filtered.length
      ? filtered
          .map(
            (c, i) =>
              `<li role="option" data-i="${i}" aria-selected="${i === selected}"><span>${c.label}</span><span>${c.hint}</span></li>`,
          )
          .join("")
  ...
  addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape") closePalette();
      ...
      e.preventDefault();
      selected =
        (selected - 1 + filtered.length) % Math.max(filtered.length, 1);
      render();
    } else if (e.key === "Enter") {
      e.preventDefault();
      runSelected();
